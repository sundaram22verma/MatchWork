...
- **Purpose**: Traditional job matching systems miss strong candidates when resume wording doesn't match job description buzzwords (for instance, candidate text stating _"led a small team through a product launch"_ failing to surface for a posting requiring _"management experience"_). MatchWork solves this by computing cosine similarity over dense vector embeddings.
- **Problems Solved**:
...
2. **Install Dependencies**:
   ...
   _(Note: This automatically triggers `npm run postinstall`, which runs `scripts/patch-tanstack-start.js` to ensure TanStack Start compiler compatibility)._
...
| `SUPABASE_PUBLISHABLE_KEY` / `SUPABASE_ANON_KEY`           | **Yes**              | The Supabase public/anonymous key used by client and authenticated server requests.                                                         |
...
| `VITE_SUPABASE_PUBLISHABLE_KEY` / `VITE_SUPABASE_ANON_KEY` | **Yes**              | Client-side Vite environment variable mapping to the Supabase public/anonymous key.                                                         |
...
1. **Build Production Artifacts**:
   ...
   npm run build
   ```
2. **Set Environment Variables**: Configure production environment variables (`SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY` or `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `HF_TOKEN`).
...
_Note: Screenshots should be placed in `./docs/screenshots/` once generated._
...
- **Fix**: Ensure your `.env` file exists in the root directory and contains `SUPABASE_URL` and either `SUPABASE_PUBLISHABLE_KEY` / `SUPABASE_ANON_KEY`. Restart your development server (`npm run dev`).
